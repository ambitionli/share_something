...
export interface AutonomousReplayScenario {
  road: RoadModel;
...
const buildings: BuildingModel[] = [
  { id: 'b-left-1', side: 'left', worldY: 8, width: 10, height: 32, color: '#64748b' },
  { id: 'b-right-1', side: 'right', worldY: 12, width: 12, height: 26, color: '#94a3b8' },
  { id: 'b-left-2', side: 'left', worldY: 28, width: 11, height: 38, color: '#475569' },
  { id: 'b-right-2', side: 'right', worldY: 34, width: 9, height: 30, color: '#64748b' },
  { id: 'b-left-3', side: 'left', worldY: 52, width: 13, height: 42, color: '#334155' },
  { id: 'b-right-3', side: 'right', worldY: 58, width: 11, height: 34, color: '#78909c' },
  { id: 'b-left-4', side: 'left', worldY: 82, width: 12, height: 36, color: '#546e7a' },
...
const cameras: CameraModel[] = [
  { id: 'front', name: '前视 Camera', fovDegrees: 110 },
  { id: 'left', name: '左视 Camera', fovDegrees: 95 },
  { id: 'right', name: '右视 Camera', fovDegrees: 95 },
  { id: 'rear', name: '后视 Camera', fovDegrees: 120 },
];
...
function roadLidarPoints(ego: EgoState): LidarPoint[] {
  const laneOffsets = [-5.4, -3.6, -1.8, 0, 1.8, 3.6];
...
function cameraFeedsFor(frameIndex: number, visibleObstacles: ObstacleModel[]): CameraFeed[] {
  ...
  return cameras.map((camera) => ({
    ...
    title: camera.name,
    status: 'recording',
    frameLabel: `Frame ${String(frameIndex + 1).padStart(2, '0')} | FOV ${camera.fovDegrees} deg`,
    detectedObstacleIds,
  }));
}

function buildFrame(keyframe: ReplayKeyframe, frameIndex: number): ReplayFrame {
  ...
  const radarDetections = radarDetectionsFor(visibleObstacles, keyframe.ego);
  ...
  return {
    ...
    lidarPoints,
    radarDetections,
    cameraFeeds: cameraFeedsFor(frameIndex, visibleObstacles),
  };
}

export const autonomousReplayScenario: AutonomousReplayScenario = {
  road,
  buildings,
  obstacles,
  cameras,
...
export function getReplayFrame(frameIndex: number): ReplayFrame {
  const lastIndex = autonomousReplayScenario.frames.length - 1;
  const safeIndex = Math.min(Math.max(Math.trunc(frameIndex), 0), lastIndex);

  return autonomousReplayScenario.frames[safeIndex];
}

export function getReplayStats(frame: ReplayFrame) {
