...
| `admin-web/src/utils/autonomousReplay.ts` | 自动驾驶 Webviz 的示例回放帧、点云汇总、毫米波雷达汇总、时间格式化与障碍物风险分类 |
...
- `admin-web/src/pages/AutonomousWebviz.tsx`：自动驾驶回放 Webviz 静态演示页，使用内置 replay frames 展示道路、建筑物、障碍物、激光雷达点云、毫米波雷达目标、车辆遥测与随帧变化的四路 camera 画面；当前不依赖后端 API。
...
- 2026-05-24：补充 Webviz camera 帧变化与毫米波雷达目标说明（修改时间：2026-05-24 06:46 UTC）。
